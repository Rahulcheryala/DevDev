import { task, logger, configure } from "@trigger.dev/sdk/v3";
...
configure({
  secretKey: process.env["tr_dev_sLAcTSuzo7laMONeNT6d"], // starts with tr_dev_ or tr_prod_
});

const resend = new Resend("re_ZGNxhCh3_2BLuEPEXWMkrSJV8B1gm48ZS");
    ...
    while (!dynamicsDataExists) {
      try {
        ...
        const { data: existingData, error: existingDataError } = await supabase
          .from("trigger_data_already_processed_rows")
        ...
        const linesResponse = await axios.get(
          ...
          }/${mainFieldName}?$filter=${headerFieldToFilter} eq '${header[`${headerFieldToFilter}`]
          }'`,
          {
            headers: { Authorization: `Bearer ${accessToken}` },
          },
        ...
        for (const line of linesData) {
          ...
          try {
            logger.info(
              `Evaluating for line ${header[headerFieldToFilter]
...
const job = task({
  ...
  run: async () => {
    try {
      ...
      logger.info(`Workflow retrieves data: ${data[0]}`);
      if (data && data[0]) {
