...
import { type DepartmentListType } from "~/modules/access-settings";
...
const AllDepartments = ({ data, count }: DepartmentTableProps) => {
    ...
    <div className={`${false ? "pr-[336px]" : ""} relative`}>
      <div className="flex items-center justify-between">
        ...
        <div className="grid grid-flow-col auto-cols-max items-center">
          ...
          <Button
            variant="blue"
            onClick={() => navigate(path.to.departmentsNew)}
      ...
      {count === 0 ? (
        <div className="mx-auto max-w-[450px]">
          <EmptyList
            ...
            ctaHandle={() => navigate(path.to.departmentsNew)}
          />
        </div>
      ) : (
        <DepartmentTable data={data} count={count} />
      )}
    </div>
  );
};
