...
export default function SearchResults({ systemDefinedMasterLists }: ListViewProps) {
    const { masterListView } = useMasterlistStore()

    const { data: searchResults, isPending, isError } = useSearchMasterlist();
    ...
    if (!searchResults || searchResults?.masterlist?.length === 0) {
    ...
    return (
        <ScrollArea className="h-[calc(100vh-420px)]">
            ...
            <div className="space-y-[2px]">
                ...
                {masterListView === "All" ? searchResults?.masterlist?.map((masterList: any) => (
                    <ListCard
                    ...
                    : masterListView === "User Defined" ? searchResults?.masterlist?.map((masterList: any) => (
                        <ListCard
                            name={masterList.name}
                            id={masterList.id}
                            key={masterList.id}
