import { Image, StatusPill } from "@zeak/ui";

export interface IConnectionListItemRecord{
  id: string;
  logo: string;
  name: string;
...
interface ConnectionListItemProps {
  record: IConnectionListItemRecord;
  selectedId: string;
  onItemClicked: (record: IConnectionListItemRecord) => void;
}
    ...
    >
      <div className="flex items-center gap-4">
        <Image
          src={record.logo}
          alt={record.name}
          className={`h-10 w-10 min-h-10 min-w-10 p-2 object-cover group-hover:bg-white rounded-zeak ${selectedId === record.id ? "bg-white" : "bg-[#00000010]"}`}
        />
        <div className="flex-1">
          <p className="flex justify-between items-center">
            <span
              title={record.name}
              className={`text-[14px] font-medium truncate max-w-[180px] ${selectedId === record.id ? "text-white" : "text-secondary group-hover:text-white"}`}
            >
              {record.name}
            </span>
            ...
            <StatusPill
              ...
              className="p-0"
              textColor={`${selectedId === record.id && "brightness-150"}`}
              iconColor={`${selectedId === record.id && "brightness-150"}`}
              uppercase
            />
          </p>
          <p
