        ...
        let sorted = records
            .filter(record => {
                ...
                if (activeStatus === 'Archived') {
                    ...
                    return record.status === activeStatus && !record.isArchived;
                }
                return !record.isArchived;
            })
            .filter(record => {
                ...
                <div className="tab-section flex gap-3">
                    {status.map((statusItem) => (
                        <button
                            ...
                            className={`text-accent-dark py-[10px] px-3 rounded-[12px] transition-colors cursor-pointer
        ...
        <div className='mt-3'>
            <div className='flex justify-between items-center px-4 py-[14px] bg-white rounded-t-[12px]'>
                <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                    ...
                    <PopoverContent className="w-48 p-0 py-1" side="bottom" align="start">
                        {sortOptions.map((option) => (
                            <div
                                key={option.id}
                                className={`flex items-center gap-2 px-4 text-left py-2 text-sm hover:rounded-[12px] hover:bg-[#B7DCFF]
                                    ${sortBy === option.id ? 'bg-[#B7DCFF] rounded-[12px]' : 'text-secondary'}`}
                                onClick={() => {
                                    setSortBy(option.id);
            ...
            <div className='relative'>
                <div className="listing-container h-[calc(100vh-430px)] pb-16 overflow-auto">
                    {filteredRecords.length > 0 ? (
                        filteredRecords.map(record => (
                            <div
                                key={record.id}
                                onClick={() => onItemClicked(record)}
                                className={`px-4 py-3 mt-[2px] cursor-pointer hover:bg-accent-primary group
                                    ${selectedId === record.id ? 'bg-accent-primary' : 'bg-white '}`}
                            >
                ...
                <button
                    ...
                    className='flex justify-center items-center gap-3 absolute bottom-0 w-full px-7 py-5 bg-[#0D0844] text-white rounded-[12px] hover:bg-[#1a1255] transition-colors transform hover:scale-[.98]'
                >
                    {button}
                </button>
            </div>
