...
import type { DropdownItem } from "~/components/Globals/Dropdown";
...
export function DataTable<TData, TValue>({
    ...
    const handleActionsClick = (event: React.MouseEvent, rowId: string) => {
        ...
        setDropdownPositions(prev => ({
            ...
            [rowId]: {
                ...
                left: rect.left + window.scrollX - 200 // offset to position dropdown to the left
            }
        }));
        setActiveRowActions(activeRowActions === rowId ? null : rowId);
    };
    ...
    useEffect(() => {
        const updatePositions = () => {
            if (activeRowActions) {
                const button = document.querySelector(`[data-row-id="${activeRowActions}"]`);
                if (button) {
                    const rect = button.getBoundingClientRect();
                    setDropdownPositions(prev => ({
                        ...prev,
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <div className="rounded-xl border border-[#EAECF0] relative">
                <Table>
                    <TableHeader>
                        ...
                        {table.getRowModel().rows?.length ? (
                            table.getRowModel().rows.map((row) => (
                                ...
                                >
                                    ...
                                    <TableCell
                                        className={cn(
                                            "p-[16px] flex-none w-auto relative",
                                            row.getIsSelected() && "bg-[#007AF5]/10"
                                        )}
                                    ...
                                    >
                                        <div className="relative" ref={actionsRef}>
                                            ...
                                            {activeRowActions === row.id && otherFunctions?.length > 0 && (
                                                <div style={{
                                                    position: 'fixed',
                                                    top: `${dropdownPositions[row.id]?.top || 0}px`,
                                                    left: `${dropdownPositions[row.id]?.left || 0}px`,
                                                    zIndex: 9999
                                                }}>
                                                    <Dropdown items={otherFunctions.map(item => ({
                                                        ...item,
                            ...
                            <TableEmptyState title={"Click to create"} link={"/x/access-settings/companies/new"} />
                        )}
                    </TableBody>
                </Table>
            </div>
