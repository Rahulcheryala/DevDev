...
<a href="https://www.loom.com/share/7b2dccab3d404b84aa8f08e5bfa21d16?sid=56583b4c-2ae4-4b2a-b402-187a11b84922" target="_blank">
  <img src="https://github.com/barbinbrad/zeak/assets/64510427/229985c9-44de-4b2b-afdd-7d1f4fd9dedd" width="70%" />
</a>
</p>

# XcelPros ERP

XcelPros is a high performance, open-source, single tenant (incomplete) ERP written in Typescript. It allows customers, suppliers, and employees to share a common platform that's easy to integrate with.
...
| ------------ | --------------- | --------------------- |
| `zeak`       | ERP Application | `npm run dev`         |
...
| -------------------- | ----------------------------------------------------------------------- |
| `eslint-config-zeak` | Shared, extendable eslint configuration for apps and packages           |
| `@zeak/database`     | Database schema, migrations and types                                   |
| `@zeak/documents`    | Transactional PDFs and email templates                                  |
| `@zeak/jest`         | Jest preset configuration shared across apps and packages               |
| `@zeak/logger`       | Shared logger used across apps                                          |
| `@zeak/react`        | Shared web-based UI components                                          |
| `@zeak/redis`        | Redis cache client                                                      |
| `@zeak/tsconfig`     | Shared, extendable tsconfig configuration used across apps and packages |
| `@zeak/utils`        | Shared utility functions used across apps and packages                  |
...
| Application     | URL                                                                                                                |
| --------------- | ------------------------------------------------------------------------------------------------------------------ |
| XcelPros ERP    | [http://localhost:3000](http://localhost:3000)                                                                     |
...
2. Provide the Redis URL in the .env file:
...
For example, to run test command in the `@zeak/react` package you can run:

```bash
$ npm run test -w @zeak/react
```

To seed the Trigger Categories, Integrations, events and actions tables with default values, you can run after the application has started and a new company and user has been created:

```bash
$ npx tsx packages/database/src/seed-triggers.ts
```
...
### To perform db migration after creating table in supabase
...
###Realtime applied on following supabase tables ->
1. notfQueue
2. integrations
3. workflowLogs


Some rpc functions are applied on following supabase tables ->