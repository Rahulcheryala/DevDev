...
export function createLayer(
  ...
  const item = layerInfo.item || new SceneItem();

  // Set the initial transform properties at time 0
  item.set(0, "transform", {
    translate: `${0}px, ${translateY}px`,
  });
  return {
    scope: [],
...
export default class LayerManager extends GroupManager {
  ...
  public setLayers(
    layers: ScenaElementLayer[],
    groups: ScenaElementLayerGroup[] = this.groups,
  ) {
    this.layers = layers;
  ...
  public findChildren(
    parentScope: string[] = [],
  ): Array<ScenaElementLayerGroup | ScenaElementLayer> {
    ...
    children = children.filter((child, i, arr) => {
      if (child.type === "group") {
        ...
        return !arr.find(
          (nextChild, j) =>
            j < i && nextChild.type === "group" && nextChild.id === id,
        );
      }
      return true;
    });
  ...
  public setCSS(
    ...
    cssObject: string | Record<string, any>,
  ) {
    layer.item.set(0, cssObject);
  }
  public setCSSByElement(
    element: HTMLElement | SVGElement,
    cssObject: string | Record<string, any>,
  ) {
  ...
  public filterDisplayedLayers<
    T extends ScenaElementLayer | ScenaElementLayerGroup,
  >(layerGroups: T[]): T[] {
    return layerGroups.filter((layerGroup) => {
      return this.compositeStyle(layerGroup).display !== "none";
    });
  }
  public filterDisplayedTargets(
    childs: Array<GroupArrayChild | GroupSingleChild>,
  ): TargetGroupsType {
    return childs
      .map((child) => {
        if (child.type === "single") {
  ...
  public getLayerById(id: string) {
    if (!id) return;
    return this.layers.filter((layer) => layer.id === id);
  }
}
