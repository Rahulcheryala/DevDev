...
export const EffectiveDates: React.FC<EffectiveDatesProps> = ({
    ...
    const getDisplayLabel = (date: CalendarDate | null, isEndDate: boolean = false) => {
        if (isEndDate && doesNotExpire) return `Does not expire ${date ? `(${date?.toString()})` : ""}`;
        if (!date) return "";
        if (isToday(date)) return `Today (Default) ${date ? `(${date?.toString()})` : ""}`;
                ...
                <div className="grid grid-cols-2 gap-12 py-4 px-4">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <DatePicker
                            ...
                            minValue={minEndDate ? minEndDate : startDate ? startDate.add({ days: 1 }) : minStartDate}
                            inputClasses="bg-[#ffffff]"
