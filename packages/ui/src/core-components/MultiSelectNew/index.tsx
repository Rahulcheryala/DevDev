...
const ItemCard = ({ item, onRemove }: { item: Item; onRemove: () => void }) => (
    <div className="flex h-[72px] px-3 items-center justify-between bg-[#FFDF41] rounded-xl">
                ...
                <div className="bg-white rounded-xl shadow-[0px_0px_15px_0px_rgba(223,229,242,0.80)] p-4 min-w-[515px] max-w-[515px]">
                    ...
                    <div className="mt-4 divide-y divide-[#E4E7EC]">
                        {filteredItems.map(item => (
                            <div
                                key={item.id}
                                className={`flex h-[72px] px-6 items-center gap-4 hover:bg-[#F0F4FD] ${selectedItems.find(i => i.id === item.id) ? 'bg-[#F0F4FD]' : ''}`}
                            >
                                <Checkbox
