// "use client"";
import { getColor } from "@zeak/utils";
...
export const avatarVariants = cva(
  "inline-flex overflow-hidden rounded-full items-center justify-center text-white font-semibold",
  {
    variants: {
      size: {
        "2xl": "h-32 w-32 min-w-[128px] text-6xl",
        eighty: "h-[80px] w-[80px] min-w-[80px] text-2xl",
        xxl: "h-[100px] w-[100px] min-w-[100px] text-3xl",
  ...
  ({ className, name, src, size, children, ...props }, ref) => {
    ...
    return src && !error ? (
      ...
      <span
        ...
        style={name ? { backgroundColor: "white" || background } : undefined}
        {...props}
        ref={ref}
      >
        <>
          {avatarInitials ? (
            <span className="text-[#00FF7B]" style={{ color: '#00FF7B' || color }}>
              {avatarInitials}
            </span>
          ) : (
            <svg
